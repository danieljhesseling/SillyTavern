 ...
 * @param {string} source
 * @param {string} file
 * @returns {{kind: string, data: any, where: string}[]}
 */
function blocksOf(source, file) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            out.push({ kind: match[1], data: parsed?.[match[1]] ?? {}, where: `${file}:${i + 1}` });
        } catch (error) {
            throw new Error(`${file}:${i + 1}: el bloque «${match[1]}» no es YAML válido — ${error.message.split('\n')[0]}`);
        }
        i = j - 1;
    }
    return out;
...
function readGuion(dir) {
    ...
    for (const file of files) {
        for (const block of blocksOf(readFileSync(new URL(file, dir), 'utf8'), file)) {
            ...
            if (!id) throw new Error(`${block.where}: un bloque «${block.kind}» sin id`);
...
function buildPack(g, catalogue) {
    ...
    const locations = places.map(l => ({
        ...
        routes: (l.caminos ?? []).map((/** @type {any} */ r) => ({ to: placeName(r.a), days: Number(r.dias) || 1 })),
    }));
    ...
    const pack = {
        ...
        rumors,
        abilities,
        plot: { title: text(world.nombre), milestones, endings },
...
const { files, byKind } = readGuion(dir);
...
for (const issue of found.warnings) console.log(`AVISO  ${issue.path}: ${issue.message}`);
for (const issue of found.errors) console.log(`ERROR  ${issue.path}: ${issue.message}`);
